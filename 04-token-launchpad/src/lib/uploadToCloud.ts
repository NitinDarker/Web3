...
const uploadToCloud = async (jsonData: MetadataJson): Promise<string> => {
  ...
  if (!CLOUDNAME) {
    throw new Error(
      "Cloud Name is not configured. Set CLOUDINARY_CLOUD_NAME in environment variables."
    );
  }
  ...
  try {
    ...
    if (!response.data || !response.data.secure_url) {
      throw new Error(
        "Invalid response from Cloudinary. Missing secure_url in response data."
      );
    }
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (axiosError.response) {
        console.error(
          `Cloudinary upload failed with status ${axiosError.response.status}:`,
        ...
        throw new Error(
          "No response received from Cloudinary server. Check your network connection."
        );
      } else if (axiosError.code === "ECONNABORTED") {
        throw new Error("Request to Cloudinary timed out. Try again later.");
      } else {
        console.error(
          "Error setting up Cloudinary request:",
          axiosError.message
        );
        throw new Error(
          `Error preparing Cloudinary upload: ${axiosError.message}`
        );
      }
    } else {
